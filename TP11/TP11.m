...
for i=1:4
    ...
    DF = 1./(1+dt*F_paths(:,9-i,9-i));

    for h=1:length(Hs) 
        
        CF_2period = TwoPeriodSwationCF(V_1,V_2,Hs(h),DF);
    ...
    %Optimal H
    [argvalue, argmax] = max(SP); OptimalH(i) = Hs(argmax);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 % L(T_{x-1},T_x)
 CF(exercise_idx(:,end),end) = V_2(exercise_idx(:,end),1)...
                            .* DF(exercise_idx(:,end),1);
